...
class Dataset:
    def __init__(self, train_list, test_list, database_root, store_memory=True, data_aug=False, data_aug_scales=[0.5, 0.8, 1]):
        ...
        self.flows_train = []
        self.flows_train_path = []
        self.labels_train = []
        self.labels_train_path = []
        for idx, line in enumerate(train_paths):
            if store_memory:
                img = Image.open(os.path.join(database_root, str(line.split()[0])))
                flow = Image.open(os.path.join(database_root, str(line.split()[1])))
                label = Image.open(os.path.join(database_root, str(line.split()[2])))
                if data_aug:
                    ...
                    slef.flows_train.append(np.array(flow, dtype=np.uint8))
            ...
            self.flows_train_path.append(os.path.join(database_root, str(line.split()[1])))
            self.labels_train_path.append(os.path.join(database_root, str(line.split()[2])))
        sys.stdout.write('\n')
        self.images_train_path = np.array(self.images_train_path)
        self.flows_train_path = np.array(self.flows_train_path)
        ...
        for idx, line in enumerate(test_paths):
            if store_memory:
                self.images_test.append(np.array(Image.open(os.path.join(database_root, str(line.split()[0]))),
                                                 dtype=np.uint8))
                self.flows_test.append(np.array(Image.open(os.path.join(database_root, str(line.split()[1]))),
                                                dtype=np.uint8))
                if (idx + 1) % 1000 == 0:
                    print('Loaded ' + str(idx) + ' test images')
            self.images_test_path.append(os.path.join(database_root, str(line.split()[0])))
            self.flows_test_path.append(os.path.join(database_root, str(line.split()[1])))
        ...
        if phase == 'train':
            ...
            if self.test_ptr + batch_size < self.test_size:
                if self.store_memory:
                    ...
                    flows = self.flows_test[self.test_ptr:self.test_ptr + batch_size]
                ...
                if self.store_memory:
                    ...
                    flows = self.flows_test[self.test_ptr:] + self.flows_test[:new_ptr]
                paths = self.images_test_path[self.test_ptr:] + self.images_test_path[:new_ptr]
                self.test_ptr = new_ptr
            return images, flows, paths
        else:
